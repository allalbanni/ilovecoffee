...
class MarkingEngine:
    ...
    def _parse_markscheme(self, markscheme) -> Dict[str, str]:
        ...
        # If it's already a dict (from JSON), normalize keys and values
        if isinstance(markscheme, dict):
            return {str(key): str(value) for key, value in markscheme.items()}

        # If it's a list, convert a list of answers to numbered questions
        if isinstance(markscheme, list):
            for index, item in enumerate(markscheme, start=1):
                if isinstance(item, dict):
                    for key, value in item.items():
                        expected_answers[str(key)] = str(value)
                else:
                    expected_answers[str(index)] = str(item)
        ...
        if isinstance(markscheme, str):
            ...
            try:
                ...
                if isinstance(parsed, list):
                    return self._parse_markscheme(parsed)
